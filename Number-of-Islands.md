...
汎用性のあるqueue1を使ったやり方を選択した。今回はBFSを使ったが、DFSでもかけるらしい。
具体的にはキューをスタックにかえる。
FIFOとFILOで性質が変わり、BFSは最短経路を保証し、DFSは深さを優先するとのこと（こっちのメリットはあまりわからない。使用メモリが少ない？らしい？）
空間計算量も、時間計算量も変わらないが、中身が違うとのこと。
ここを問題ごとに使い分けることも要求されるのだろうか・・・？